...
public class Ranges implements Iterable<Interval> {
  ...
  private void replaceIfOverlapping(Interval toRemove, ListIterator<Interval> iter, Interval current) {
    if (current.overlaps(toRemove)) {
      ...
      iter.remove();
      replaceOverlappingInterval(iter, lowerPart, upperPart);
    }
  }

  private void replaceOverlappingInterval(ListIterator<Interval> iter, Interval lowerPart, Interval upperPart) {
    addIfNonEmpty(iter, lowerPart);
    addIfNonEmpty(iter, upperPart);
  }

  private void addIfNonEmpty(ListIterator<Interval> iter, Interval part) {
    if (part.width() > 0.0) {
      iter.add(part);
    }
  }
